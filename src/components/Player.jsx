  ...
  return (
    <LinearGradient colors={[ "#006400", "green"]} style={styles.playerContainer}>
      ...
      <Pressable style={styles.info} onPress={() => navigate("TrackScreen")}>
        ...
        <Text style={styles.subtitle}>{track.channel}</Text>
      </Pressable>
      <View style={styles.controls}>
        <Pressable style={styles.playButton} onPress={() => (player.playing? player.pause() : player.play())}>
          {player.paused? <FontAwesome5 name="play" size={24} color="white" /> : <FontAwesome5 name="pause" size={24} color="white" />}
        </Pressable><Pressable style={styles.playButton} onPress={() => moveForward()}>
